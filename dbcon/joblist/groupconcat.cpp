...
void GroupConcatInfo::mapColumns(const RowGroup& projRG)
{
	...
	for (vector<SP_GroupConcat>::iterator k = fGroupConcat.begin(); k!= fGroupConcat.end(); k++)
	{
		...
		(*k)->fRowGroup = RowGroup(oids.size(), pos, oids, keys, types, scale, precision);
...
void GroupConcatAgUM::initialize()
{
	...
	fGroupConcat->fRowGroup.initRow(&fRow);
...
void GroupConcatOrderBy::processRow(const rowgroup::Row& row)
{
	...
	else if (fOrderByCond.size() > 0 && fRule.less(row.getData(), fOrderByQueue.top().fData))
	{
		...
		fCurrentLength -= *((int16_t*)(fRow1.getData()));
...
void GroupConcatOrderBy::merge(GroupConcator* gc)
{
	...
	while (go->fOrderByQueue.empty() == false)
	{
		...
		if (fDistinct && fDistinctMap->find(row.fData + 2) != fDistinctMap->end())
		{
			; // no op;
		}
		...
		else if (fCurrentLength < fGroupConcatLen)
		{
			...
			fCurrentLength += *((int16_t*)row.fData);
		...
		{
			OrderByRow swapRow = fOrderByQueue.top();
			fCurrentLength -= *((int16_t*)(swapRow.fData));
...
void GroupConcatOrderBy::getResult(uint8_t* buff, string sep)
{
	ostringstream oss;
	bool addSep = false;
...
void GroupConcatNoOrder::getResult(uint8_t* buff, string sep)
{
	ostringstream oss;
	bool addSep = false;
	...
	{
		fRowGroup.setData(fDataQueue.front().get());
		fRowGroup.getRow(0, &fRow);
