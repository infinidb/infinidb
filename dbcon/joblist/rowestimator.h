...
public:
	...
	* @param scanFlags vector of flags with one entry per extent populated by the casual partitioning evaluation, each 
	*                  that will be scanned are true, the ones that were eliminated by casual partitioining are false.
	...
	*/
	...
	/** @brief Estimate the number of rows that will be returned for a particular table given a ColumnCommandJL for non 
	*          casual partitioning column.  Added for bug 3503.
	*
...
private:
	...
	*
	* This function works for multiple operations against the same column such as "col1 = 5 or col1 = 10".  It 
	* calls estimateOpFactor for each individual operation.
	*
