...
/***********************************************************************
*   $Id: functioncolumn.cpp 8436 2012-04-04 18:18:21Z rdempsey $
*
*
***********************************************************************/
#include <string>
...
FunctionColumn::FunctionColumn(const string& functionName, const string& funcParmsInString, const u_int32_t sessionID):
    ReturnedColumn(sessionID),
    fFunctionName(functionName),
    fData (functionName + "(" + funcParmsInString + ")"),
    fFunctor(0)
{
    functionParms (funcParmsInString);   
}

FunctionColumn::FunctionColumn( const FunctionColumn& rhs, const u_int32_t sessionID):
    ...
    fFunctor(rhs.fFunctor)
{
	...
	for (uint i = 0; i < rhs.fFunctionParms.size(); i++)
	{
...
const string FunctionColumn::toString() const
{
  ostringstream output;
	...
	output << "resultType=" << fResultType.colDataType << "|" << fResultType.colWidth << endl;
	output << "operationType=" << fOperationType.colDataType << endl;
...
void FunctionColumn::functionParms (const string& funcParmsInString)
{
...
{    
	...
	b << static_cast<u_int32_t>(fFunctionParms.size());
	for (uint i = 0; i < fFunctionParms.size(); i++)
