...
int ProcessCommandStatement(THD *thd, string& dmlStatement, cal_connection_info& ci, std::string schema="")
{
	...
	//MySQL server pukes if rollback ever fails. If DMLProc isn't running, rollback will fail.
	if (dmlStatement == "ROLLBACK") isRollback = true;
	if (dmlStatement == "COMMIT") isRollback = false;
