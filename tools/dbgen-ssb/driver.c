...
FILE* lineorder_partitions[PARTITIONS];
...
usage (void)
{
	...
	fprintf (stderr, "-x     -- don't partition lineorder table\n");
	fprintf (stderr,
	  ...
	  case 'T':				/* generate a specifc table */
		  ...
		  case 'O':				/* optional actions */
			  ...
			  case 'h':				/* something unexpected */
				  fprintf (stderr,
					  "%s Population Generator (Version %d.%d.%d%s-Calpont)\n",
...
void close_partitions(void)
{
	...
	for (i = 0; i < PARTITIONS; ++i)
	{
		if (lineorder_partitions[i]) fclose(lineorder_partitions[i]);
	}
}
