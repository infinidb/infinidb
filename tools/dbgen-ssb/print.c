 ...
 */
    ...
    *ptr = 0;
    ...
    lineorder_partitions[partition] = f;
    return (f);
}
...
pr_line(order_t *o, int mode)
{
    ...
    for (i = 0; i < o->lines; i++)
    {
	    ...
	    {//@bug 808 partition files
			memcpy(year, o->lineorders[i].orderdate, 4);
			year[4] = '\0';
			memcpy(mon, o->lineorders[i].orderdate + 4, 2);
			mon[2] = '\0';

			partition = (atoi(year) - STARTPARTITION) * 12 + atoi(mon);
			fp_l = lineorder_partitions[partition];
