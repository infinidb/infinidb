...
FILE  *
open_partition(int tbl, int partition, char* year, char* mon)
{
    char      filename[256];
    char      fullpath[256];
    char      path[256];
...
/*
 * tdefs.name looks like lineorder.tbl or lineorder.tbl.1
 * We want to make it look like lineorder_199201.tbl or lineorder_199201.tbl.1
 */

    strcpy(filename, tdefs[tbl].name);
    ptr = strchr(filename, '.');
    assert(ptr);
    *ptr = 0;
    ...
    {
        sprintf(fullpath, "%s_%s%s.%s", filename, year, mon, (ptr + 1));
    }
    else 
    {
        sprintf(path, "%s%c%s", env_config(PATH_TAG, PATH_DFLT), PATH_SEP, filename);
        sprintf(fullpath, "%s_%s%s.%s", path, year, mon, (ptr + 1));
    }

    f = fopen(fullpath, "w");
    OPEN_CHECK(f, fullpath);
    if (LINE == tbl)
    	line_partitions[partition] = f;
    else if (ORDER == tbl)
	order_partitions[partition] = f;
    return (f);
}

int 
get_partition(char* pdate)
{
    	FILE *fp = NULL;
	char year[5];
    	char mon[3];
	...
	memcpy(year, pdate, 4);
	year[4] = '\0';
	memcpy(mon, pdate + 5, 2);
	mon[2] = '\0';
	return (atoi(year) - STARTPARTITION) * 12 + atoi(mon);
...
pr_order(order_t *o, int mode)
{
    ...
    if (file_partition)
    {	//@bug 808 partition files
	memcpy(year, o->odate, 4);
	year[4] = '\0';
	memcpy(mon, o->odate + 5, 2);
	mon[2] = '\0';
	partition = (atoi(year) - STARTPARTITION) * 12 + atoi(mon);
	fp_o = order_partitions[partition];

	if (NULL == fp_o)
		fp_o = open_partition(ORDER, partition, year, mon);
    }
    else
...
pr_line(order_t *o, int mode)
{
    ...
    for (i = 0; i < o->lines; i++)
	    ...
	    {//@bug 808 partition files
			...
			year[4] = '\0';
			memcpy(mon, o->l[i].sdate + 5, 2);
			mon[2] = '\0';

			partition = (atoi(year) - STARTPARTITION) * 12 + atoi(mon);
			fp_l = line_partitions[partition];

			if (NULL == fp_l)
				fp_l = open_partition(LINE, partition, year, mon);
	    }
