/*
* $Id: driver.c,v 1.5 2008/03/21 17:38:39 jms Exp $
*
* Revision History
* ===================
* $Log: driver.c,v $
...
/* @bug 808 file print in partitions */
FILE  *open_partition (int tbl, int partition, char* year, char* mon);
/*
...
void initialize_partition_files(void);
FILE  * open_partition_file(int tbl, int year, int mon);
void close_partitions(void);

FILE* line_partitions[PARTITIONS];
FILE* order_partitions[PARTITIONS];
...
usage (void)
{
	...
	fprintf (stderr, "-x     -- don't partition lineitem and orders tables\n");
	fprintf (stderr,
...
process_options (int count, char **vector)
{
	...
	while ((option = getopt (count, vector,
		"b:C:Dd:Ffi:hn:O:P:qr:s:S:T:U:vx")) != -1)
	switch (option)
		{
	  ...
	  case 'T':				/* generate a specifc table */
		  ...
		  case 'O':				/* optional actions */
			  ...
			  case 'F':				/* generate flat files for later loading */
				  direct = 0;
				  break;
			  case 'x':				/* partition lineitem and orders tables (@bug 808) */
				  file_partition = 0;
				  break;
			  ...
			  case 'h':				/* something unexpected */
				  fprintf (stderr,
					  "%s Population Generator (Version %d.%d.%d build %d-Calpont)\n",
...
FILE  *
open_partition_file(int tbl, int year, int mon)
{
    char      filename[256];
    char      fullpath[256];
    char      path[256];
    ...
    len = strlen(tdefs[tbl].name) - 4;
    memcpy(filename, tdefs[tbl].name, len);
    filename[len] = '\0';

    if (*tdefs[tbl].name == PATH_SEP)
	sprintf(fullpath, "%s_%d%02d.tbl", filename, year, mon);

    else 
    {
        sprintf(path, "%s%c%s", env_config(PATH_TAG, PATH_DFLT), PATH_SEP, filename);
	sprintf(fullpath, "%s_%d%02d.tbl", path, year, mon);
    }
...
void initialize_partitions(void)
{
	...
	{
		line_partitions[i] = NULL;
		order_partitions[i] = NULL;
	}
}
...
void close_partitions(void)
{
	...
	for (i = 0; i < PARTITIONS; ++i)
	{
		if (line_partitions[i]) fclose(line_partitions[i]);
		if (order_partitions[i]) fclose(order_partitions[i]);
	}
}
 ...
 */
	for (i = PART; i <= REGION; i++)
		...
		time(&endtime);
		time_elapsed = difftime(endtime, starttime);
		printf("Program run time: %2.0f seconds\n", time_elapsed);
