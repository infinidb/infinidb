/******************************************************************************
 * $Id: timeset.h 4512 2008-10-15 15:18:55Z dcathey $
 *
 *****************************************************************************/
...
#ifndef BROADCASTTIMESET_H
#define BROADCASTTIMESET_H
...
void TimeSet::displayAll() const
{
	...
	{
		double t =  (double)it->second.tv_sec +
					(double)it->second.tv_nsec / 1000000000.0;
		std::cout << "TimeSet " << it->first << ": "   << std::fixed << t << "\n";
	}
	std::cout.flush();
}
...
void TimeSet::display(const std::string& key) const
{
	...
	if (fElapsed.end() != em)
	{
		double t =  (double)em->second.tv_sec +
	...
	else
	{
		...
		if (fTimer.end() != tm)
		{
			double t =  (double)tm->second.tv_sec +
...
double TimeSet::totalTime() const
{
	...
	double totSeconds = (double)tSum.tv_sec + (double)tSum.tv_nsec/1000000000.0;
